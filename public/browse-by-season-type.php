...
<main class="container">
    <section class="row justify-content-center mb-5">
        ...
        <h2 class="display-5">Browse by Season Type</h2>
        ...
        <?php
        $category_query = "SELECT DISTINCT season_type FROM travel_destinations WHERE season_type IS NOT NULL";
        $category_result = $connection->query($category_query);

        $category = [];
        while ($row = $category_result->fetch_assoc()) {
            $category[] = $row['season_type'];
        }
        ...
        if (count($category) > 0) {
            ...
            foreach ($category as $season_type) {
                if ($counter % 5 == 0 && $counter > 0) {
            
                    echo "</div><div class=\"row justify-content-center\">";
                }
                 // Is this button currently 'pressed'?
                $is_active = in_array($season_type, $active_filters['season_type'] ?? []);
                // Let's use our custom function to build the query string we need and retain any values we have. 
                $url = build_query_url($_SERVER['PHP_SELF'], $active_filters, 'season_type', $season_type);

                echo "<div class=\"col-2 mb-2\">"; 
                echo "<a href=\"$url\" class=\"btn " . ($is_active ? 'btn-outline-success' : 'btn-success') . " w-100\" aria-pressed=\"" . ($is_active ? 'true' : 'false') . "\">$season_type</a>";
                echo "</div>";
        ...
        <div class="m-6">
            ...
            <?php 
                ...
                if (isset($active_filters['season_type'])) {
                    $season_type_values = $active_filters['season_type'];
                    $placeholders = str_repeat("?,", count($season_type_values) - 1) . "?";
                    $sql .= " AND season_type IN ($placeholders)";
                    $types .= str_repeat("s", count($season_type_values));
                    $parameters = array_merge($parameters, $season_type_values);
                }
