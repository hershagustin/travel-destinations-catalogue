...
foreach ($_GET as $filter => $values) {
    if ($filter === 'destination_category') { 
...
<main class="container">
    <section class="row justify-content-center mb-5">
        ...
        <h2 class="display-5">Browse by Destination Category</h2>
        ...
        <?php
        $category_query = "SELECT DISTINCT destination_category FROM travel_destinations WHERE destination_category IS NOT NULL";
        $category_result = $connection->query($category_query);

        $category = [];
        while ($row = $category_result->fetch_assoc()) {
            $category[] = $row['destination_category'];
        }
        ...
        if (count($category) > 0) {
            ...
            foreach ($category as $destination_category) {
                if ($counter % 5 == 0 && $counter > 0) {
            
                    echo "</div><div class=\"row justify-content-center\">";
                }
                 // Is this button currently 'pressed'?
                $is_active = in_array($destination_category, $active_filters['destination_category'] ?? []);
                // Let's use our custom function to build the query string we need and retain any values we have. 
                $url = build_query_url($_SERVER['PHP_SELF'], $active_filters, 'destination_category', $destination_category);

                echo "<div class=\"col-2 mb-2\">"; 
                echo "<a href=\"$url\" class=\"btn " . ($is_active ? 'btn-outline-success' : 'btn-success') . " w-100\" aria-pressed=\"" . ($is_active ? 'true' : 'false') . "\">$destination_category</a>";
                echo "</div>";
        ...
        <div class="m-6">
            ...
            <?php 
                ...
                if (isset($active_filters['destination_category'])) {
                    $destination_category_values = $active_filters['destination_category'];
                    $placeholders = str_repeat("?,", count($destination_category_values) - 1) . "?";
                    $sql .= " AND destination_category IN ($placeholders)";
                    $types .= str_repeat("s", count($destination_category_values));
                    $parameters = array_merge($parameters, $destination_category_values);
                }
